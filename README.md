...
* [EqualEval语音识别能力排行榜（2024年9月）](#asr-all)
* [EqualEval语音识别能力（无方言版）排行榜（2024年9月）](#asr-no-dialect)
* [EqualEval语音生成能力排行榜（2024年9月）](#tts-all)
* [EqualEval语音生成能力（无方言版）排行榜（2024年9月）](#tts-no-dialect)
...
4.  EqualEval大模型语音能力评测榜涵盖语音识别能力、语音生成能力两个一级维度，30+子维度，全方面评估大模型的语音能力。语音识别能力评测采用字符错误率（Character Error Rate，CER）作为主要的评测指标，来评估语音识别准确性。为更方便地展示各大模型语音识别的性能，我们将CER转换为正确率进行直观的展示。语音生成能力评测使用词错误率（Word Error Rate, WER）作为客观指标，在评测WER时使用开源模型Paraformer-zh模型进行非实时语音识别，保证了评测的公平性。同时加入主观指标拟人度和自然度(包括语义一致性、音质、表现力)进行人工评分，以保证评测的全面性。
5. 我们会定期更新评测数据集并推出新的评测结果，随着数据集难度的调整以及评测维度的增加，评测分数会发生相应变化。
    

【2024-9-18】，发布EqualEval公能中文大模型排行榜基础能力及智能体能力榜单。

【2024-10-5】，发布EqualEval语音大模型能力排行榜。


<a name="basic-all"></a>

### EqualEval基础能力总排行榜（2024年9月）

| 排名 | 模型名称 | 机构 | 是否开源 | 总分 | 语言理解与生成 | 知识能力 | 数学能力 | 推理能力 | 代码能力 | 对话体验 | 安全性 | 回答一致性 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Doubao-pro-32k | 字节跳动 | 否 | 83.75 | 83.13 | 81.32 | 84.64 | 83.15 | 82.46 | 64.30 | 91.79 | 99.20 |
...
| 8 | Vicuna-13B-v1.5 | 斯坦福 | 是 | 33.84 | 27.33 | 21.76 | 14.01 | 25.73 | 20.88 | 51.59 | 42.91 | 60.00 |
...
### EqualEval语音识别能力（无方言版）排行榜（2024年9月）

| 排名 | 模型/接口名称        | 机构           | 是否开源 | 总分  | 朗读场景 | 对话与语音输入 |
| --- | ------------------- | -------------- | -------- | ---- | -------- | -------------- |
...
### EqualEval语音生成能力（无方言版）排行榜（2024年9月）
